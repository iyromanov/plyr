...
| `ads`                | Object                     | `{ enabled: false, preLoad: true, publisherId: '', tagUrl: '' }`                                                               | `enabled`: Whether to enable advertisements. `preLoad`: if set to true, ads will be requested immediately, otherwise only on `play` event. `publisherId`: Your unique [vi.ai](https://vi.ai/publisher-video-monetization/?aid=plyrio) publisher ID. `tagUrl` is a URL for a custom VAST tag if you're not using Vi.                                                                                                                                                                             |
